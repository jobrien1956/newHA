// Trip.js - Last updated: 2026-03-23 08:57 - Trip page renderer - oninit/onbeforeupdate reload fix, footer Instagram only, redirect support, margin-top navbar fix, backgroundImg basePath fix
HA.views.Trip = {
		...
		function header(trip) {
			...
			if (bgRaw) {
				if (bgRaw.startsWith('http')) {
					...
					// strip any leading ../ or newHA/
					bgRaw = bgRaw.replace(/^(\.\.\/)+/, '').replace(/^newHA\//, '');
